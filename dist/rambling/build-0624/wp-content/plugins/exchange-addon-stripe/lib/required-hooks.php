...
function it_exchange_stripe_plugin_row_actions( $actions, $plugin_file, $plugin_data, $context ) {
    $actions['setup_addon'] = '<a href="' . get_admin_url( NULL, 'admin.php?page=it-exchange-addons&add-on-settings=stripe' ) . '">' . __( 'Setup Add-on', 'it-l10n-exchange-addon-stripe' ) . '</a>';    
    return $actions;
}
...
function it_exchange_stripe_addon_enqueue_script() {
    ...
    wp_localize_script( 'stripe-addon-js', 'stripeAddonL10n', array(
            'processing_payment_text'  => __( 'Processing payment, please wait...', 'it-l10n-exchange-addon-stripe' ),
        )
    );
}
...
function it_exchange_stripe_addon_process_transaction( $status, $transaction_object ) {
	...
	if ( ! empty( $_REQUEST['_stripe_nonce'] ) && ! wp_verify_nonce( $_REQUEST['_stripe_nonce'], 'stripe-checkout' ) ) {
		it_exchange_add_message( 'error', __( 'Transaction Failed, unable to verify security token.', 'it-l10n-exchange-addon-stripe' ) );
		return false;
	}
	...
	if ( ! empty( $_POST['stripeToken'] ) ) {
		...
		try {
			...
			if ( $subscription_id ) {
				...
				if ( empty( $stripe_customer->subscription->plan->name ) || $subscription_id != $stripe_customer->subscription->plan->name ) {
					...
					throw new Exception( __( 'Error: You cannot subscribe to the same plan!', 'it-l10n-exchange-addon-stripe' ) );	
				}
			} else {
		...
		it_exchange_add_message( 'error', __( 'Unknown error. Please try again later.', 'it-l10n-exchange-addon-stripe' ) );
	}
	return false;
...
function it_exchange_cancel_stripe_subscription( $subscription_details ) {
	...
	try {
		...
		it_exchange_add_message( 'error', sprintf( __( 'Error: Unable to unsubscribe user %s', 'it-l10n-exchange-addon-stripe' ), $e->getMessage() ) );
	}
}
	...
	if ( $subscription ) {
		...
		if ( !is_object( $stripe_plan ) ) {
			...
			try {
				...
				return sprintf( __( 'Error: Unable to create Plan in Stripe - %s', 'it-l10n-exchange-addon-stripe' ), $e->getMessage() );
			}
...
function it_exchange_stripe_addon_transaction_status_label( $status ) {
    switch ( $status ) {
        case 'succeeded':
            return __( 'Paid', 'it-l10n-exchange-addon-stripe' );
        case 'refunded':
            return __( 'Refunded', 'it-l10n-exchange-addon-stripe' );
        case 'partial-refund':
            return __( 'Partially Refunded', 'it-l10n-exchange-addon-stripe' );
        case 'needs_response':
            return __( 'Disputed: Stripe needs a response', 'it-l10n-exchange-addon-stripe' );
        case 'under_review':
            return __( 'Disputed: Under review', 'it-l10n-exchange-addon-stripe' );
        case 'won':
            return __( 'Disputed: Won, Paid', 'it-l10n-exchange-addon-stripe' );
        case 'cancelled':
            return __( 'Cancelled', 'it-l10n-exchange-addon-stripe' );
        default:
            return __( 'Unknown', 'it-l10n-exchange-addon-stripe' );
    }
}
	...
	foreach ( $cart_object->products as $product ) {	
		...
		if ( $autorenews ) {
			...
			switch( $status ) {
				...
				case 'deactivated':
					$output = __( 'Recurring payment has been deactivated', 'it-l10n-exchange-addon-stripe' );
					break;
					
				case 'cancelled':
					$output = __( 'Recurring payment has been cancelled', 'it-l10n-exchange-addon-stripe' );
					break;
				
				case 'active':
				default:
					$output  = '<a href="' .  add_query_arg( array( 'it-exchange-stripe-action' => 'unsubscribe-user', 'it-exchange-stripe-customer-id' => $stripe_customer_id, 'it-exchange-stripe-subscriber-id' => $subscriber_id ) ) . '">' . __( 'Cancel Recurring Payment', 'it-l10n-exchange-addon-stripe' ) . '</a>';
					break;
			}
			?>
